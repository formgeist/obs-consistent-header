...
var INFINITY_RADIUS = 1e4; // far enough for a sub-2px precision on a 4k screen, good enough for text bounds; 64 bit floats still work well with it
...
function angleToCircline(midRadius, alpha, direction /* 1 for clockwise and -1 for anticlockwise circline */) {
...
function getSectorRowGeometry(ringSector, cx, cy, totalRowCount, linePitch, rowIndex, fontSize, rotation) {
    ...
    var cheapTangent = Math.max(0, to - from); /* Math.tan(Math.max(0, to - from)) */ // https://en.wikipedia.org/wiki/Small-angle_approximation
...
function getAllBoxes(rawTextGetter, valueFormatter, sizeInvariantFontShorthand, valueFont, node) {
    return rawTextGetter(node)
        .split(' ')
        .map(function (text) { return (__assign({ text: text }, sizeInvariantFontShorthand)); })
        .concat(valueFormatter(node[group_by_rollup_1.AGGREGATE_KEY])
        .split(' ')
...
function fill(config, layers, fontSizes, measure, rawTextGetter, formatter, textFillOrigins, shapeConstructor, getShapeRowGeometry, getRotation) {
    return function (node, index) {
        ...
        var allBoxes = getAllBoxes(rawTextGetter, valueFormatter, sizeInvariantFont, valueFont, node);
...
function fillTextLayout(measure, rawTextGetter, valueFormatter, childNodes, config, layers, textFillOrigins, shapeConstructor, getShapeRowGeometry, getRotation) {
    ...
    return childNodes.map(fill(config, layers, fontSizes, measure, rawTextGetter, valueFormatter, textFillOrigins, shapeConstructor, getShapeRowGeometry, getRotation));
}
