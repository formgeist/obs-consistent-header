...
function linkTextLayout(measure, config, nodesWithoutRoom, currentY, anchorRadius, rawTextGetter, valueFormatter) {
    ...
    return nodesWithoutRoom
        .filter(function (n) { return n.depth === maxDepth; }) // only the outermost ring can have links
        ...
        var _a = measure(linkLabel.fontSize, [
            __assign(__assign({ fontFamily: config.fontFamily }, linkLabel), { text: text }),
        ])[0], width = _a.width, emHeightAscent = _a.emHeightAscent, emHeightDescent = _a.emHeightDescent;
        return {
            link: [
                [x0, y0],
                [stemFromX, stemFromY],
            ...
            text: text,
            valueText: valueFormatter(node[group_by_rollup_1.AGGREGATE_KEY]),
            width: width,
